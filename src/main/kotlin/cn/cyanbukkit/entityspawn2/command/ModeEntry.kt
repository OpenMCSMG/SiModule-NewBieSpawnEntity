...
import cn.cyanbukkit.entityspawn2.GameHandle
import cn.cyanbukkit.entityspawn2.utils.Mode
import org.bukkit.command.Command
import org.bukkit.command.CommandSender
import org.bukkit.entity.Player

object ModeEntry : Command("entitymode") {
        ...
        for (method in methods) {
            ...
            if (mode != null) {
                if (mode.value == args[0]) {
                    ...
                    val convertedArgs = anise(info, parameterTypes, sender)
                    method.invoke(hand, *convertedArgs)
                    return true
                }
            }
        }
        sender.sendMessage("§c未找到模式")
        return true
    }
